...
img = cv2.imread("481544_Football players_degraded-HW4.png", cv2.IMREAD_GRAYSCALE)
...
horizontal_line[:, int((filter_size+1)//2-1):int((filter_size+1)//2+2)] = 1
rotate_kernel = rotate_image(horizontal_line, -45)
plt.imshow(np.abs(rotate_kernel), 'gray')
plt.show()
result = WienerFilter(img, rotate_kernel, 0.001)
